...
export function NurseVitalHistoryPage() {
  ...
  return <div className="workspace-page"><div className="back-link"><Link to={`/nurse/patients/${patientId}`}><ArrowLeft size={16} /> Back to patient overview</Link></div><PageHeader eyebrow="Patient monitoring" title={patient.data ? `${patient.data.full_name} · Vital history` : 'Vital history'} description="Measurements, classifications, and the configured explanations used at the time." actions={<Link className="button button--primary" to={`/nurse/patients/${patientId}/vitals/new`}><Plus size={16} /> Record vitals</Link>} /><PatientPageState pending={patient.isPending} error={patient.error} />{patient.data && <VitalHistory patientId={patientId} />}</div>
}
