...
- ICE candidates are persisted the same way (`ice_candidates`), and `GET /calls/ice/` returns the STUN/TURN servers the browser should use. Configure `WEBRTC_TURN_URLS`/credentials for calls between browsers on different networks; STUN-only is best-effort and commonly cannot relay through symmetric NAT or corporate firewalls.
